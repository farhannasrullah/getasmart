...
function openWhatsApp(type, productName = '') {
  ...
  if (type === 'general') {
    message =
      "Halo GetasMart, saya ingin bertanya mengenai produk dan layanan UMKM Desa Getas.";
  } else if (type === 'partnership') {
    message =
      "Halo BUMDes Mitra Mandiri, saya tertarik untuk mendiskusikan peluang kemitraan bisnis dengan UMKM Desa Getas.";
  } else if (type === 'order') {
...
function largeCardHTML(id, p) {
  return `
    <div
      ...
      class="catalog-item cursor-pointer glass-card rounded-2xl md:rounded-3xl md:col-span-8 overflow-hidden group relative flex flex-col md:flex-row reveal"
    >
      <div class="w-full md:w-[61.8%] relative aspect-golden md:aspect-auto md:h-full overflow-hidden skeleton-bg image-wrapper">
        <img
          class="lazy-image w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
          data-src="${esc(p.imgMain)}"
          alt="${esc(p.name)}"
          loading="lazy"
        />
      </div>
      ...
      <div class="w-full md:w-[38.2%] p-5 md:p-8 flex flex-col justify-center bg-white/20 dark:bg-black/20 flex-grow">
        ...
        <span class="inline-block bg-secondary-container/50 dark:bg-secondary-container/20 text-on-secondary-container dark:text-secondary-fixed font-semibold text-[10px] md:text-xs px-2 md:px-3 py-1 rounded-full w-fit mb-3 md:mb-4">
          ${esc(p.badge)}
        </span>

        <h2 class="text-2xl md:text-3xl font-bold text-primary mb-2 md:mb-3">
          ${esc(p.name)}
        </h2>

        <p class="text-sm md:text-base text-on-surface-variant mb-6 md:mb-8 desc-clamp">
          ${esc(p.desc)}
        </p>
        ...
        <div class="flex items-center justify-between mt-auto pt-4 border-t md:border-none border-primary/5">
          ...
          <button
            data-wa-order="${esc(id)}"
            aria-label="Pesan ${esc(p.name)} via WhatsApp"
            class="liquid-btn w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center text-on-secondary-fixed shadow-md"
          >
            <span
...
function normalCardHTML(id, p) {
  return `
    <div
      data-product-id="${esc(id)}"
      data-category="${esc(p.category)}"
      class="catalog-item cursor-pointer glass-card rounded-2xl md:rounded-3xl md:col-span-4 overflow-hidden group flex flex-col hover:shadow-lg transition-shadow reveal"
    >
      ...
      <div class="p-5 md:p-6 flex flex-col flex-grow bg-white/10 dark:bg-black/20">
        ...
        <h3 class="text-lg md:text-xl font-bold text-primary mb-1 md:mb-2">
          ${esc(p.name)}
        </h3>

        <p class="text-on-surface-variant text-xs md:text-sm mb-4 md:mb-6 desc-clamp">
          ${esc(p.desc)}
        </p>

        <div class="flex items-center justify-between mt-auto pt-3 border-t border-primary/5">

          <span class="text-base md:text-lg text-secondary dark:text-secondary-fixed font-bold">
            ${formatPrice(p.price)}
            <span class="text-[10px] md:text-xs font-normal">
              ${esc(p.unit)}
            </span>
          </span>
          ...
          <button
            data-wa-order="${esc(id)}"
            aria-label="Pesan ${esc(p.name)} via WhatsApp"
            class="w-10 h-10 md:w-12 md:h-12 rounded-full bg-white flex items-center justify-center text-primary shadow-sm hover:scale-110 transition-transform"
          >
            <span
              class="material-symbols-outlined text-[20px] md:text-[24px]"
              aria-hidden="true"
...
function homeLargeCardHTML(id, p) {
  return `
    <div
      data-product-id="${esc(id)}"
      class="cursor-pointer glass-card rounded-2xl md:rounded-3xl p-5 md:p-8 flex flex-col justify-end relative overflow-hidden group md:col-span-2 aspect-[4/3] sm:aspect-[1.618/1] md:aspect-auto reveal"
    >
      ...
      <div class="relative z-20 transform transition-transform duration-500 group-hover:-translate-y-2 mt-auto">
        ...
        <span class="px-2 md:px-3 py-1 bg-white/20 text-white rounded-full text-[10px] md:text-xs font-semibold backdrop-blur-md mb-2 md:mb-3 inline-block">
          ${esc(p.badge || 'Best Seller')}
        </span>

        <h3 class="text-2xl md:text-3xl font-bold text-white mb-1 md:mb-2">
          ${esc(p.name)}
        </h3>

        <p class="text-white/80 text-xs md:text-base w-[85%] md:w-3/4 desc-clamp">
          ${esc(p.desc)}
        </p>
...
function homeSmallCardHTML(id, p) {
  return `
    <div
      data-product-id="${esc(id)}"
      class="cursor-pointer glass-card rounded-2xl md:rounded-3xl p-5 md:p-6 flex flex-col justify-end relative overflow-hidden group aspect-square md:aspect-[4/3] reveal"
    >
      ...
      <div class="relative z-20 mt-auto">
        ...
        <h3 class="text-base md:text-xl font-bold text-white mb-0.5 md:mb-1">
          ${esc(p.name)}
        </h3>

        <p class="text-white/80 text-[11px] md:text-sm desc-clamp">
          ${esc(p.shortDesc || p.desc)}
        </p>
...
function renderHomeFeatured(list) {
  ...
  if (smalls.length) {
    html += `
      <div class="grid grid-cols-2 gap-3 md:flex md:flex-col md:gap-6 md:col-span-1">
        ${smalls
          .map(({ id, data }) =>
            homeSmallCardHTML(id, data)
          )
          .join('')
        }
      </div>
...
document
  .getElementById('home-featured-wrap')
...
function renderCatalog(list) {
  ...
  if (!list.length) {
    container.innerHTML = `
      <p class="col-span-12 text-center text-on-surface-variant py-10">
        Belum ada produk.
        Tambahkan lewat
        <a
          href="/admin.html"
  ...
  container.innerHTML = list
    .map(({ id, data }) =>
      data.size === 'large'
        ? largeCardHTML(id, data)
        : normalCardHTML(id, data)
    )
    .join('');
}
...
function showProduct(id) {
  ...
  [
    ...
    if (
      imgEl.parentElement &&
      imgEl.parentElement.classList.contains(
        'image-wrapper'
      )
    ) {
      imgEl.parentElement.classList.add(
        'skeleton-bg'
      );
    }
  });
...
function toggleFaq(btnElement) {
  const item =
    btnElement.closest('.faq-item');

  const wasOpen =
    item.classList.contains('faq-open');
...
window.addEventListener('load', () => {
  ...
  setTimeout(() => {
    ...
    */
    ...
    /*
      Kalau halaman terakhir adalah DETAIL PRODUK,
      tunggu PRODUCTS selesai tersedia.
    */
    ...
    if (lastPage === 'product') {
      ...
      if (
        ...
        /*
          Kalau produk sudah tidak ada /
          gagal dimuat, fallback ke katalog.
        */
      ...
      /*
        ...
        home / catalog / village / about / faq
      */
...
function showPage(
  ...
  /*
     Jika pindah dari detail produk
     ke halaman biasa, kita tidak hapus
     lastProduct agar masih bisa digunakan
     saat diperlukan.
  */


  currentPageIndex =
    pageOrder.indexOf(name) !== -1
      ? pageOrder.indexOf(name)
      : 0;
...
/* =========================================================================
   ...
   (rAF-throttled for smoother low-end performance)
========================================================================= */
